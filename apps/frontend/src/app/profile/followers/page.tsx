...
import { getFollowersForUser, unfollowUser } from '@/lib/api';
...
export default function FollowersPage() {
  ...
  const loadFollowers = async () => {
    try {
      ...
      if (token && user) {
        const allFollowers = await getFollowersForUser(user.uid, token);
  ...
  const handleUnfollow = async (userId: string) => {
    try {
      ...
      if (token) {
        ...
        setFollowers(followers.filter(u => u.uid !== userId));
      }
    } catch (error) {
