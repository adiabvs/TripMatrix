...
import { MdDelete, MdLocationOn, MdStar, MdFavorite, MdChatBubbleOutline, MdMoreVert, MdPerson, MdSend, MdEdit, MdClose, MdCheck } from 'react-icons/md';
import { formatCurrency, getCurrencySymbol } from '@/lib/currencyUtils';
import { likePlace, unlikePlace, getPlaceLikes, getPlaceComments, addPlaceComment, updatePlaceComment, deletePlaceComment, getUser } from '@/lib/api';
...
export default function CompactStepCard({ 
  ...
  const handleStartEdit = (comment: PlaceComment) => {
  ...
  const handleSaveEdit = async (commentId: string) => {
    ...
    try {
      const token = await getIdToken();
      const updatedComment = await updatePlaceComment(place.placeId, commentId, editCommentText.trim(), token);
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      ...
      const token = await getIdToken();
      await deletePlaceComment(place.placeId, commentId, token);
